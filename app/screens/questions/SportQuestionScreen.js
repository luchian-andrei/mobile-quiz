...
import sportQuestions from "../../components/database/sportQuestions";

export default function SportQuestionScreen() {
  const params = useLocalSearchParams();

  const [qIndex, setQIndex] = useState(0);
  const [selectedAnswer, setSelectedAnswer] = useState(-1);
  const [score, setScore] = useState(0);
  const correctAnswerIndex = sportQuestions[qIndex].correctAnswer;
  const correctAnswerText = sportQuestions[qIndex].correctAnswerText;
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Text style={styles.welcomeMessage}>
        Topic: <Text style={styles.highlightText}>Sport</Text>
      </Text>
      {showAnswer ? (
      ...
      <Text style={styles.questionAndAnswer}>
        {sportQuestions[qIndex].question}{" "}
      </Text>
      {!check ? (
        <FlatList
          data={sportQuestions[qIndex].possibleAnswers}
